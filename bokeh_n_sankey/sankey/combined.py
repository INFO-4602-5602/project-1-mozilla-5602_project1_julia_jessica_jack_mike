...
p = figure(title = 'Mode fear response', plot_height = int(600/1.0) , plot_width = int(950/1.0), toolbar_location = None, tools = [hover])
...
if choice == 'Fear Level':
    sankey = hv.Sankey(sdata, ['From','To'], vdims=value_dim)
...
# Create Holomap
##dag_dict = {'Fear Level':generateGraph('Fear Level')}
...
##theme = Theme(json={'attrs':{'Figure':dict(toolbar_location=None)}})

renderer = hv.renderer('bokeh')
##renderer.save(hmap,'whatnow')


##widget = renderer.get_widget(hmap, 'widgets')
##renderer.theme = theme

##renderer.save(hmap,'graph.html')

# Save html file
##hv.save(hmap,'sankey.html')

# my code
##sank_plot = renderer.get_plot(hmap)#.state
sank_plot = renderer.get_plot(sankey).state
##sank_plot = renderer.get_plot(hmap)#.state
##print(type(sank_plot))
##sank_plot = renderer.get_widget(hmap, None).state
...
##        newSource = dag_dict['Importance Rankings']  # changed this to the dict
##    if select.value == 'Importance Rankings':
##        newSource = dag_dict['Fear Level']  # changed this to the dict
...
##output_file("combss.html")
save(layout)
##show(layout)


##hv.save(hmap,'combss.html')
##hv.save(layout,'combss.html')
